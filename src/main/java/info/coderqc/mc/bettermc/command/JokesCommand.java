...
import info.coderqc.mc.bettermc.procedures.JokesCommandExecutedProcedure;
...
public class JokesCommand {
	...
	public static void registerCommand(RegisterCommandsEvent event) {
		event.getDispatcher().register(Commands.literal("jokes").requires(s -> s.hasPermission(2))
				.then(Commands.argument("arguments", StringArgumentType.greedyString()).executes(arguments -> {
					...
					JokesCommandExecutedProcedure.execute(world, entity);
					return 0;
				})).executes(arguments -> {
					ServerLevel world = arguments.getSource().getLevel();
					double x = arguments.getSource().getPosition().x();
